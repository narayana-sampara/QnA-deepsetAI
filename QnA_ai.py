...
# ============================================================
# NEW: Text Cleaning (fixes glued-word artifacts from scraped snippets)
# ============================================================
def normalize_text(text: str) -> str:
    """Inserts missing spaces at word-fusion boundaries commonly introduced
    by scraped snippets, e.g. 'NarendraDamodardasModi' -> 'Narendra Damodardas Modi',
    'isthecapitalcity' won't be fully fixed by case alone, but most compound
    fusions ('PrimeMinisteroftheRepublic', '2014from2001') are lower->Upper or
    digit<->letter transitions, which this catches."""
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)   # lower -> Upper boundary
...
def clean_answer(answer: str) -> str:
    """
    Fixes cases where DDG snippets lack a space around the matched term,
    e.g. 'isNara Chandra Babu Naidu' -> 'Nara Chandra Babu Naidu'
         'Vande Mataramis' -> 'Vande Mataram'
    Trailing-word strip only applies when it leaves a multi-word phrase,
    so single dictionary words like 'Paris' or 'crisis' are left alone.
    """
    cleaned = answer.strip()

    # Leading glued word: "isNara..." -> "Nara..."
    cleaned = _GLUED_LEADING.sub("", cleaned).strip()

    # Trailing glued word: only strip if result still has a space
    # (i.e. it was a multi-word phrase, not a single real word ending in "is"/"was"/etc.)
...
def clean_summary(text: str) -> str:
    """Strips leading date-stamp junk like '2 weeks ago -' or 'December 12, 2022 -'."""
...
def web_qa(query):
    ...
    try:
        ...
        print(f"⚠️  Search failed ({e}). Try again in a moment.")
        return None
...
# CLI
# ============================================================
print("\n🚀 Guru AI v6 (Routed, Role‑Aware Intelligence)")
